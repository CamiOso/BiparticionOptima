#!/bin/bash
# Corre todas las filas Geometric pendientes de 20A de a 2 en paralelo
...
run_par() {
    ...
    echo "========================================"
    echo "[GEO-COLA] Lanzando fila $FILA1 (start_k=$SK1) ${FILA2:+y fila $FILA2} — $(date)"
    echo "========================================"
    python3 -u run_geo_single.py $FILA1 --start-k $SK1 > /tmp/geo_${FILA1}cola.log 2>&1 &
    local PID1=$!
    if [ -n "$FILA2" ]; then
        python3 -u run_geo_single.py $FILA2 --start-k $SK2 > /tmp/geo_${FILA2}cola.log 2>&1 &
        wait $PID1 $!
    else
        wait $PID1
    fi
    echo "[GEO-COLA] Par terminado — $(date)"
}
