"""
IBQNodos para n=26 en Julia — versión memory-efficient.

Ventajas vs Python:
  • La TPM de 6.6 GB se memory-mapea (sin copia en RAM)
  • Los perfiles IB usan 2048 muestras en vez de 67M → sin OOM
  • marginal_directo suma solo 2^(n_free) estados, no materializa tensores

Uso:
    julia --project=. main_ibqnodos.jl
    julia --project=. main_ibqnodos.jl --desde 5   # retomar desde caso 5
    julia --project=. main_ibqnodos.jl --excel ruta/DatosIBQNodos2026.xlsx
"""
...
# ─── Configuración ────────────────────────────────────────────────────────────
const N          = 26
const SISTEMA    = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
...
const TPM_PATH  = joinpath(PROJ_DIR, "..", "src", ".samples", "N26A.npy")
const CKPT_PATH = joinpath(PROJ_DIR, "checkpoint_ibqnodos_26a.json")
EXCEL_PATH      = joinpath(PROJ_DIR, "..", "DatosIBQNodos2026.xlsx")
const TIMEOUT_S  = 3600   # 1 hora por caso
...
# ─── Leer casos desde JSON (evita XLSX.jl que no lee bien este archivo Excel) ─
const CASOS_JSON = joinpath(PROJ_DIR, "casos_26a.json")

function leer_casos()::Vector{Dict}
    isfile(CASOS_JSON) || error("No se encontró $CASOS_JSON. Generarlo con:\n  python3 -c \"import openpyxl,json; ...\"\n")
    raw = JSON3.read(read(CASOS_JSON, String), Vector)
    [Dict(
        "fila"    => Int(c["prueba"]) + 5,   # fila Excel = prueba + 5 (datos desde fila 6)
...
# ─── Crear hoja resultado (skip: XLSX.jl incompatible con este archivo) ───────
function inicializar_hoja_resultados()
    println("  [INFO] Resultados → checkpoint JSON + resultados_26a.json (Excel se actualiza con Python al final)")
end

# ─── Escribir resultado (solo checkpoint; Python actualiza Excel al final) ─────
function escribir_resultado(fila::Int, prueba::Int, alc_str::String, mec_str::String,
    ...
    # Los resultados ya quedan en el checkpoint JSON.
    # Al terminar todos los casos, ejecutar: python3 actualizar_excel_26a.py
end
...
function main()
    ...
    for (i, arg) in enumerate(ARGS)
        ...
        elseif arg == "--excel" && i < length(ARGS)
            global EXCEL_PATH = abspath(ARGS[i+1])
        end
    end

    # Cargar TPM mapeada
    tpm_path = abspath(TPM_PATH)
    isfile(tpm_path) || error("TPM no encontrada: $tpm_path\nEjecuta primero: python scripts/generar_N26A.py")
    ...
    println("\n$("="^60)")
    println("  26A Julia: $(length(casos)) casos | completados: $(length(done_filas))")
    println("$("="^60)\n")
    ...
    for (i, caso) in enumerate(casos)
        ...
        try
            escribir_resultado(caso["fila"], prueba, caso["alc_str"], caso["mec_str"],
                               phi_val, elapsed, particion)
        catch ex_xlsx
            println("    AVISO: No se pudo escribir en Excel: $ex_xlsx")
        end
        ...
        if idx !== nothing
            resultados[idx] = caso_res
        else
            push!(resultados, caso_res)
        end
    ...
    # Escribir JSON de resultados para que Python actualice el Excel
    res_path = joinpath(PROJ_DIR, "resultados_26a.json")
    write(res_path, JSON3.write(Dict("casos" => resultados)))
    println("  Resultados guardados en: $res_path")
    println("  Para actualizar el Excel: python3 actualizar_excel_26a.py")
end
