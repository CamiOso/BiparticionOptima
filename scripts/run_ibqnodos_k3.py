"""Benchmark IBQNodos k=3 vs QNodos k=3 en DatosPruebas2026_1.xlsx.

Uso:
    python run_ibqnodos_k3.py                    # todas las hojas
    python run_ibqnodos_k3.py --hojas 10A 15B   # solo esas hojas
    python run_ibqnodos_k3.py --desde 15B --fila 7  # retomar desde fila 7 de 15B
"""
...
EXCEL_OUT   = PROJECT / "DatosIBQNodos_k3_2026.xlsx"
CHECKPOINT  = PROJECT / "checkpoint_ibqnodos_k3.json"
...
def leer_casos(cfg: dict) -> list[dict]:
    """Lee todos los casos con phi QNodos k=3 definido (col 11) de la hoja."""
    ...
    for r in range(6, ws.max_row + 1):
        ...
        phi_ref = ws.cell(r, 11).value   # QNodos k=3 pérdida
        t_ref   = ws.cell(r, 12).value   # QNodos k=3 tiempo
        if alc is None:
            break
        if phi_ref is None:
            continue
...
def crear_excel_resultado(resultados: dict) -> None:
    ...
    for hoja_key, casos in resultados.items():
        if not casos:
            continue
        ws = wb.create_sheet(title=f"{hoja_key}-IBQNodos-k3")
        ws.append([
            "#Fila", "Alcance", "Mecanismo",
            "QNodos_k3_φ", "QNodos_k3_t(s)",
            "IBQNodos_k3_φ", "IBQNodos_k3_t(s)",
            "Speedup", "Match_φ",
            "Partición_IBQNodos_k3",
        ])
        for caso in casos:
...
def run_hoja(hoja_key: str, checkpoint: dict, desde_fila: int = 0) -> list[dict]:
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"  Hoja {hoja_key} (k=3): {len(casos)} casos con QNodos k=3 ref", flush=True)

    if not casos:
        print("  Sin casos con referencia QNodos k=3 — saltando.", flush=True)
        return []
    ...
    for i, caso in enumerate(casos):
        ...
        try:
            ...
            if abs(phi_ib - caso["phi_ref"]) < 1e-5:
                match_str = "✓"
            elif phi_ib < caso["phi_ref"]:
                match_str = "↓mejor"
            else:
                match_str = "✗"
            ...
            print(
                ...
                f"{match_str}  t={elapsed:.1f}s  speedup=×{speedup}",
                flush=True,
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark IBQNodos k=3 vs QNodos k=3.")
    ...
    parser.add_argument("--desde",  default=None,
                        help="Primera hoja desde la que empezar.")
    parser.add_argument("--fila",   type=int, default=0,
                        help="Fila desde la que empezar en la primera hoja.")
    args = parser.parse_args()

    hojas_orden    = list(HOJAS.keys())
    hojas_a_correr = args.hojas
    ...
    for hoja_key, casos in resultados.items():
        ...
        print(
            ...
            f"match={len(matches)} mejor={len(better)} | "
            f"speedup avg=×{avg_sp:.1f} max=×{max_sp:.1f}"
        )
