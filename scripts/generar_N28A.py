"""Genera la TPM N28A.npy para n=28 nodos.

Formato idéntico a N27A: float32, shape=(2^28, 28), valores en (0,1).
Cada celda = P(nodo_i=1 en t+1 | estado del sistema en t).

Generación en bloques de 2^20 filas (~117 MB c/u) para no saturar la RAM.
Semilla fija (42) para reproducibilidad.

Uso:
    python scripts/generar_N28A.py
"""
...
N       = 28
ROWS    = 1 << N            # 268_435_456
COLS    = N
DTYPE   = np.float32
SEED    = 42
CHUNK   = 1 << 20           # 1_048_576 filas por bloque ≈ 117 MB
OUT     = Path("src/.samples/N28A.npy")

OUT.parent.mkdir(parents=True, exist_ok=True)

total_gb = ROWS * COLS * np.dtype(DTYPE).itemsize / 1e9
print(f"Generando N28A.npy — shape=({ROWS:,}, {COLS}), {total_gb:.2f} GB")
