...
from src.constantes.models import VARIACIONAL_LABEL
...
class ParticionVariacional(SIA):
    """Partición espectral variacional con dos operadores alternativos.

    modo="laplaciano"   — Laplaciano normalizado L_n = D^{-½}·L·D^{-½}.
                          Minimiza el corte normalizado (Shi & Malik 2000).

    modo="biharmonico"  — Operador de Schrödinger discreto H = L + γ·diag(V).
                          V[i] = 2·E[X_i] − 1 ∈ [−1, 1] es el potencial Airy:
                            V ≈ −1 (zona oscilante)  cuando P(X_i=1) ≈ 0,
                            V ≈ +1 (zona evanescente) cuando P(X_i=1) ≈ 1,
                            V =  0 (punto de retorno) cuando P(X_i=1) = 0.5.
                          Los cruces por cero del vector de Fiedler de H
                          corresponden a los "turning points" de la función
                          de Airy continua que aproxima la solución local.
    """
    ...
    def __init__(
        ...
        if modo not in {"laplaciano", "biharmonico"}:
            raise ValueError(
                f"modo debe ser 'laplaciano' o 'biharmonico', se recibio '{modo}'."
            )
        self.modo = modo
        self.gamma = gamma
    ...
    # ------------------------------------------------------------------
    # Conductancias (idéntico a Circuito — acoplamiento TPM)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Operadores espectrales: variacional vs. biarmónico (Airy)
    # ------------------------------------------------------------------

    def _operador(self, W: np.ndarray, nodos: list[int]) -> np.ndarray:
        """Devuelve el operador espectral según el modo configurado."""
        d = W.sum(axis=1)
        L = np.diag(d) - W

        if self.modo == "laplaciano":
            # Laplaciano normalizado: L_n = D^{-½} L D^{-½}
            d_safe = np.where(d > 1e-12, d, 1.0)
            D_inv_sqrt = np.diag(1.0 / np.sqrt(d_safe))
            return D_inv_sqrt @ L @ D_inv_sqrt

        # modo == "biharmonico": H = L + γ·diag(V)
        V = self._potencial_airy(nodos)
        return L + self.gamma * np.diag(V)

    def _potencial_airy(self, nodos: list[int]) -> np.ndarray:
        """V[i] = 2·P(X_i=1) − 1, promediado sobre todos los estados de la TPM.

        Mapea la probabilidad marginal de activación al rango [−1, 1]:
        nodos siempre inactivos (P=0) → V=−1 (zona oscilante de Airy),
        nodos siempre activos  (P=1) → V=+1 (zona evanescente de Airy),
        nodos en equilibrio    (P=0.5) → V=0  (punto de retorno / corte).
        """
        V = np.zeros(len(nodos))
        n_cols = self.tpm.shape[1]
        for idx, nodo in enumerate(nodos):
            if nodo < n_cols:
                V[idx] = 2.0 * float(self.tpm[:, nodo].mean()) - 1.0
        return V
    ...
    def _resolver_biparticion(
        ...
        candidatos = self._candidatos(nodos, alcance_total, mecanismo_total)
    ...
    def _candidatos(
        ...
        # Laplaciano normalizado: ev0 es trivial (vector constante) → empezar en 1.
        # Biarmónico: el potencial Airy rompe la simetría traslacional, ningún
        # eigenvector es trivial → incluir ev0 (puede ser el estado fundamental).
        inicio = 0 if self.modo == "biharmonico" else 1
        for ev_idx in range(inicio, min(inicio + 4, n)):
    ...
    def _resolver_k(
        ...
        M = self._operador(W, nodos)
        k_eff = min(k, n)
        asignacion = self._embedding_k(M, n, k_eff)
    ...
    def _embedding_k(self, M: np.ndarray, n: int, k: int) -> tuple[int, ...]:
        if n <= 1:
            return (0,) * n
        try:
            eigenvalores, eigenvectores = np.linalg.eigh(M)
    ...
    def _refinar_local(
        ...
        for _ in range(self._max_iter_refinamiento):
            vecinos = self._vecinos(
                actual.subalcance, actual.submecanismo, alcance_total, mecanismo_total
            )
            if not vecinos:
                break
            mejor_vecino = actual
