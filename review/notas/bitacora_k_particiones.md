...
| 2026-05-14 | Se inició la ejecución experimental sobre redes de 20 nodos (20A-Elementos) y se tomaron decisiones de estrategia frente a los límites de escalabilidad. | Escalabilidad, gestión de memoria y selección de estrategias por tamaño de subsistema. | Se identificó que QNodos y Geometric son inviables para subsistemas de n≥18 nodos (el caso n=18 tardó más de 1 hora solo en k=2). Se aplicaron correcciones críticas de memoria: límite de caché en `NCube.memo` (`_MAX_MEMO_NCUBE=64`) y `Sistema.memo` (`_MAX_MEMO_SISTEMA=256`) para evitar consumo descontrolado de RAM. Se cargó la TPM en float32 para reducir uso de memoria a 235MB. Se completaron con QNodos los 10 casos de 20A con subsistemas de n_max=10–14 nodos y se lanzó Geometric para esos mismos 10 casos con guardado fila a fila. Se generó la muestra sintética `N22A.csv` (4194304×22, ~968MB, seed=44) e identificados los 9 casos viables de 22A-Elementos (n_max=11 y 15) para ejecutar con QNodos y Geometric. Los casos con n≥18 de ambas hojas se reservaron para la estrategia Circuito, que demostró ser ~2× más rápida y usar ~8× menos RAM que QNodos para sistemas grandes. |
